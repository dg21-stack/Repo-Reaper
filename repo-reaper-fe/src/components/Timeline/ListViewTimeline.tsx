...
import { Box, Typography, TableCell, TableRow, Menu, MenuItem, TextField, Button } from '@mui/material';
...
export default function ListViewTimeline({
  ...
  return (
    <TableRow sx={{ '&:hover': { backgroundColor: '#3a3d44' }, cursor: 'pointer' }} onClick={handleConnectorClick} onContextMenu={handleContextMenu}>
      <Menu
        ...
        >
          ...
          >
            {timelineData.slice(0, 4).map((item, index) => (
              <TimelineItem key={item.id} sx={{ minWidth: '50px', padding: 0, rotate: '-90deg' }}>
                <TimelineSeparator>
                  <TimelineDot sx={{ backgroundColor: '#7289da' }}>
                    <Brightness1OutlinedIcon fontSize="small" sx={{ color: 'white' }} />
                  </TimelineDot>
            ...
            <TimelineItem sx={{ minWidth: '50px', padding: 0, display: 'flex', justifyContent: 'center', right: 40 }}>
              <Typography variant="h6" color={timelineData.length > 3 ? "white" : "transparent"} sx={{ marginLeft: '4px' }}>
                ...
              </Typography>
            </TimelineItem>
          </Timeline>
