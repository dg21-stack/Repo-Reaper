...
import { Box, Fade, Typography, Button, TextField, Menu, MenuItem } from '@mui/material';
...
interface ICustomizedTimeline {
  ...
  timelineData: { id: string; time: string; commandHistory: { time: string; command: string }[] }[];
  isEditState: boolean;
...
export default function CustomizedTimeline({
  ...
  const shouldFadeIn = (isModalTimeline || (selectedNode.id === null && selectedTimeline === null)) && !deleteModal;

  return (
    <>
    <Fade in={shouldFadeIn} timeout={300} onClick={(e) => e.stopPropagation()}>
      <Box className="timeline-container timeline-normal" onContextMenu={handleContextMenu}>
        {/* Context Menu */}
        <Menu
        ...
        >
          {(isEditState && state) || contextEditState ? (
            ...
            <Typography variant="h4" color="white" sx={{ height: '100%', display: 'flex', alignItems: 'center' }}>
              {editedBranchName.toLocaleUpperCase()}
            </Typography>
          ) : (
            <Button onClick={handleConnectorClick} sx={{ color: 'white', '&:hover': { backgroundColor: '#4e5969' } }}>
        ...
        >
          {(isModalTimeline ? timelineData: timelineData
            .slice(0, 3))
            .map((item, index) => (
              <TimelineItem key={item.id}>
                ...
                <TimelineSeparator>
                  <IconButton onClick={(e) => handleNodeClick(index, e)}>
                    <TimelineDot
                      sx={{ backgroundColor: selectedName == item.id ? '#3700b3' : '#7289da', '&:hover': { bgcolor: '#3700b3' } }}
                    >
